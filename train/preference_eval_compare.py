"""
Compare base model vs preference adapter on eval data.

Input:
- data/eval_data.jsonl

Output:
- data/preference_report.md

Example:
python train/preference_eval_compare.py --base_model distilgpt2 --adapter_path preference_adapter
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare base model vs preference adapter")
    ...
    parser.add_argument("--adapter_path", type=str, default="preference_adapter")
    parser.add_argument("--report_path", type=str, default="data/preference_report.md")
...
def main() -> None:
    ...
    # Preference-adapted model
    pref_backbone = AutoModelForCausalLM.from_pretrained(args.base_model).to(device)
    pref_model = PeftModel.from_pretrained(pref_backbone, args.adapter_path).to(device)
    pref_model.eval()
    ...
    lines: List[str] = []
    lines.append("# Preference Eval Report")
    lines.append("")
    lines.append(f"- Eval file: `{args.eval_file}`")
    lines.append(f"- Base model: `{args.base_model}`")
    lines.append(f"- Preference adapter: `{args.adapter_path}`")
    ...
    for i, row in enumerate(rows, start=1):
        ...
        lines.append("")
        lines.append("**Preference Output**")
        lines.append(md_block(pref_output))
        lines.append("")
